...
const coordinatesRegex = /((\-?|\+?)?\d+(\.\d+)?),\s*((\-?|\+?)?\d+(\.\d+)?)/g;
const isUpperCase = (string) => /^[A-Z]*$/.test(string)
const busynessElementSelectors = ["div#i4", "div#i3", "div#i2", "div.i6w5N"]

async function getLocationsData(places) {
    ...
    for (var place of places) {
        ...
        const mapElement = await page.$(`[title="Map of ${place.name}"]`);
        let coords = []
        if (mapElement) {
            const parent = await mapElement.getProperty('parentNode');
            const propertyHandle = await parent.getProperty('href');
            const propertyValue = await propertyHandle.jsonValue();
            if (propertyValue.startsWith("https://www.google.com/maps/place/")) {
                let matches = propertyValue.match(coordinatesRegex);
                if (matches.length > 0) {
